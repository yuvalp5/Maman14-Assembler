 ...
 *
 * This module handles the first phase of assembly: macro expansion.
 * It takes an assembly source file (.as) and produces an expanded
 * file (.am) where all macro invocations have been replaced with
 * their content.
 *
 *
 */
 ...
 * Contains the macro's name and its content (all the lines between
 * "mcro" and "mcroend" directives).
 */
typedef struct {
...
/**
 * @brief Main function for pre-assembler - processes macros in source file and creates an expanded file
 *
 * This function performs the macro expansion process:
 * 1. Opens the source file and destination file
 * 2. Processes macro definitions and invocations
 * 3. Writes the expanded code to the destination file
 *
