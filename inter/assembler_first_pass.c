...
void first_pass(const char *src) {
    ...
    if (!input_pre_assembled) {
        ...
        while (fgets(line, MAX_LINE_LEN, input_pre_assembled) != NULL) {
            ...
            /* Process the line only if it's not empty, not a comment, and we have a field */
            if (field != NULL && field[0] != ';') {
                ...
                if (is_storage(field)) {
                    ...
                    if (is_symbol_flag) {
                        ...
                        if (symbol_count < MAX_SYMBOLS) {
                            ...
                            symbol_found = 0;
                            for (i = 0; i < symbol_count && !symbol_found; i++) {
                        ...
                        if (strcmp(field, ".extern") == 0) {
                            ...
                            if (extern_symbol) {
                                if (symbol_count < MAX_SYMBOLS) {
                                    ...
                                    symbol_found = 0;
                                    for (i = 0; i < symbol_count && !symbol_found; i++) {
                                        if (strcmp(symbol_table[i].symbol_name, extern_symbol) == 0) {
                                            printf("Error in line %d: Symbol '%s' already defined\n", 
                            ...
                            else { /* if the operand is missing, print an error message */
                                printf("Error in line %d: Missing operand for .extern directive\n", line_number);
                                error_count++;
                            }
                        }
                    }
                ...
                /* This is an instruction line */
                    ...
                    if (is_symbol_flag) {
                        ...
                        if (symbol_count < MAX_SYMBOLS) {
                            ...
                            if (!symbol_found) {
                                ...
                                symbol_table[symbol_count].symbol_type = SYMBOL_TYPE_CODE;
                                symbol_count++;
                            }
                        } 
                        else {
                    ...
                    if (search_command_in_table(instruction)) {
                        ...
                        /* Validate operands */
                        if (!validate_operand(operand1, search_command_in_table(instruction), 1, operand1_mode) ||
                            !validate_operand(operand2, search_command_in_table(instruction), 2, operand2_mode)) {
                            proceed_to_next_line = 1;
                            continue;
                        }
                        ...
                        word_to_binary(search_command_in_table(instruction), operand1_mode, operand2_mode, code, &code_index);
                        ...
                        if (operand1_mode >= 0) {
                            switch (operand1_mode) {
                                case 0: /* Immediate */
                                    ...
                                    break;
                                case 1: /* Direct */
                                    code[code_index++] = 0; /* Will be updated in second pass */
                                    break;
                                case 2: /* Index */
                                    code[code_index++] = 0; /* Will be updated in second pass */
                                    code[code_index++] = operand1[strlen(operand1)-2] - '0';
                                    break;
                            }
                        }
                        ...
                        if (operand2_mode >= 0) {
                            switch (operand2_mode) {
                                ...
                                case 2: /* Index */
                                    code[code_index++] = 0; /* Will be updated in second pass */
                                    code[code_index++] = operand2[strlen(operand2)-2] - '0';
                                    break;
                            }
                        }
                        
                        /* Save current IC and L values along with the machine code generated */
...
int search_command_in_table(char *command) {
    ...
    while (commands[i] != NULL) {
        if (strcmp(command, commands[i]) == 0) {
            return i + 1; /* Return index + 1 to avoid returning 0 for a match */
        }
        i++;
    }
...
int calc_num_of_words(char *instruction, char *operand1, char *operand2) {
    ...
    if (instr_idx >= 1 && instr_idx <= 5) {
        ...
        /* Single-operand instructions: clr, not, inc, dec, jmp, bne, jsr, red, prn */
    if (operand1) {
...
void word_to_binary(int instruction_index, int operand1_mode, int operand2_mode,
    ...
    /* First word: op-code (4 bits), addressing modes (4 bits), function code (4 bits) */
...
int validate_operand(char *operand, int instruction_index, int operand_num, int addressing_mode) {
    ...
    if (instruction_index == 15 || instruction_index == 16) { /* rts or stop */
        if (operand != NULL && operand_num == 1) {
            printf("Error in line %d: Unexpected operand for no-operand instruction\n", 
                   line_number);
            error_count++;
            return 0;
        }
    ...
    if (!operand) {
        ...
        } else if (instruction_index >= 1 && instruction_index <= 5) { /* Two-operand instructions */
    ...
    if ((instruction_index == 10 || instruction_index == 11 || instruction_index == 12) && /* jmp, bne, jsr */
        operand[0] == '&') {
    ...
    switch (instruction_index) {
        case 1: /* mov */
            if (operand_num == 1) { /* Source operand */
                ...
                if (addressing_mode == 0 || addressing_mode == -1) {
                    printf("Error in line %d: Invalid destination operand for mov\n", line_number);
                    error_count++;
                    return 0;
                }
            }
        ...
        case 4: /* sub */
            if (operand_num == 1) { /* Source operand */
                if (addressing_mode == -1) {
                    printf("Error in line %d: Invalid source operand for arithmetic operation\n", 
                           line_number);
                    error_count++;
                    return 0;
                }
                ...
                if (addressing_mode == 0 || addressing_mode == -1) {
                    printf("Error in line %d: Invalid destination operand for arithmetic operation\n", 
                           line_number);
                    error_count++;
                    return 0;
                }
        ...
        case 5: /* lea */
            if (operand_num == 1) { /* Source operand */
                if (addressing_mode != 1) { /* Must be direct addressing */
                    printf("Error in line %d: lea requires direct addressing for source\n", 
                           line_number);
                    error_count++;
                    return 0;
                }
                ...
                if (addressing_mode == 0 || addressing_mode == -1) {
                    printf("Error in line %d: Invalid destination operand for lea\n", 
                           line_number);
                    error_count++;
                    return 0;
                }
        ...
        case 9: /* dec */
            if (addressing_mode == 0 || addressing_mode == -1) {
                printf("Error in line %d: Invalid operand for single-operand instruction\n", 
                       line_number);
                error_count++;
                return 0;
            }
    ...
    if (addressing_mode == 2) { /* Index addressing */
        ...
        if (reg[0] != 'r' || reg[1] < '0' || reg[1] > '7' || reg[2] != ']') {
            printf("Error in line %d: Invalid register in index addressing '%s'\n", 
                   line_number, operand);
            error_count++;
            return 0;
        }
    ...
    if (strcmp(directive, ".data") == 0) {
        ...
        while (token != NULL) {
            ...
            if (!isdigit(token[0]) && token[0] != '-') {
                printf("Error in line %d: Invalid number in .data directive: '%s'\n", 
                       line_number, token);
                error_count++;
                return 0;
            }
        ...
        for (i = 0; operands[i] != '\0'; i++) {
            ...
            printf("Data[%d] = %d ('%c')\n", DC-1, operands[i], operands[i]); /* Debug output */
        }
...
void print_symbol_table(void) {
    ...
    printf("----------------------\n");
    for (i = 0; i < symbol_count; i++) {
        printf("%-16s %-8d ", symbol_table[i].symbol_name, symbol_table[i].symbol_value);
